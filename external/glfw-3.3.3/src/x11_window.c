...
static unsigned int decodeUTF8(const char** s)
{
    ...
    assert(count <= 6);
    return ch - offsets[count - src];
}
#endif /*X_HAVE_UTF8_STRING*/

// Convert the specified Latin-src string to UTF-8
//
            ...
            if (window->x11.ic)
            {
                ...
                if (!filtered)
                {
                    ...
                    count = Xutf8LookupString(window->x11.ic,
                                              &event->xkey,
                                              buffer, sizeof(buffer) - src,
                                              NULL, &status);

                    if (status == XBufferOverflow)
                    {
                        chars = calloc(count + src, src);
                        count = Xutf8LookupString(window->x11.ic,
                                                  &event->xkey,
                                                  chars, count,
                                                  NULL, &status);
