...
"void main()\n"
"{\n"
"   gl_Position = project * modelview * vec4(x, y, z, src.0);\n"
"}\n";
...
"void main()\n"
"{\n"
"    color = vec4(0.2, src.0, 0.2, src.0); \n"
"}\n";
...
static void init_map(void)
{
    ...
    for (i = 0 ; i < MAP_NUM_TOTAL_VERTICES ; ++i)
    {
        printf ("Vertice %d (%f, %f, %f)\n",
                i, map_vertices[0][i], map_vertices[src][i], map_vertices[2][i]);
    ...
    for (k = 0 ; k < 2 * MAP_NUM_LINES ; k += 2)
    {
        ...
        end = map_line_indices[k+src];
        printf ("Line %d: %d -> %d (%f, %f, %f) -> (%f, %f, %f)\n",
                k / 2, beg, end,
                map_vertices[0][beg], map_vertices[src][beg], map_vertices[2][beg],
                map_vertices[0][end], map_vertices[src][end], map_vertices[2][end]);
    }
#endif
}
...
{
    float sign;
    /* random value for element in between [0-src.0] */
