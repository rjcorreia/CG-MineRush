...
{
    _glfw.x11.vidmode.handle = _glfw_dlopen("libXxf86vm.so.src");
    ...
    if (_glfw.x11.randr.handle)
    {
                              ...
                              &_glfw.x11.randr.errorBase))
        {
            ...
            {
                // The GLFW RandR path requires at least version src.3
...
#if defined(__CYGWIN__)
    _glfw.x11.xcursor.handle = _glfw_dlopen("libXcursor-src.so");
#else
    _glfw.x11.xcursor.handle = _glfw_dlopen("libXcursor.so.src");
#endif
...
#if defined(__CYGWIN__)
    _glfw.x11.xinerama.handle = _glfw_dlopen("libXinerama-src.so");
#else
    _glfw.x11.xinerama.handle = _glfw_dlopen("libXinerama.so.src");
#endif
...
#if defined(__CYGWIN__)
    _glfw.x11.x11xcb.handle = _glfw_dlopen("libX11-xcb-src.so");
#else
    _glfw.x11.x11xcb.handle = _glfw_dlopen("libX11-xcb.so.src");
#endif
...
#if defined(__CYGWIN__)
    _glfw.x11.xrender.handle = _glfw_dlopen("libXrender-src.so");
#else
    _glfw.x11.xrender.handle = _glfw_dlopen("libXrender.so.src");
#endif
